...
20230112 Added urlPattern
*/

import { MATCH_HTML_COMMENT, MATCH_COMMENT } from "./tlfConstants";

// https://uibakery.io/regex-library/url
const partialURLPattern = /\b[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9()@:%_\+.~#?&//=]*)\b/.source;
const fullURLPattern = /\b(https?|file|s?ftp):\/\/(?:www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9()@:%_\+.~#?&\/=]*)\b/.source;

export function getWordCount(text: string, excludeURLFromWordCounts: boolean): number {
...
export function getParagraphCount(text: string): number {
  ...
  // the last paragraph doesn't get counted
	const lastCharacter = ( text || "").slice(-1);
	if ( lastCharacter != "\n" && lastCharacter != "\r" ) { paragraphs++; }
...
export function getURLFrequencyArray(text: string): [string] {
	const pattern = new RegExp(
		[
			partialURLPattern,
			fullURLPattern,
	...
	urlFrequencyArray.sort(function(a, b){
		if (a[1] > b[1]) {
			...
			// a[1] == b[1] values match, so check keys
			if (a[0] < b[0]) {
				return -1;
			} else if (a[0] > b[0]) {
				return 1;
...
export function getWordFrequencyArray(text: string, excludeURLFromWordCounts: boolean): [string, number] {

	if (excludeURLFromWordCounts) {
		text = stripURLs(text);
	}

// added 0027' 2019′ 2032’ (apostrophes) (curly apostrophes)
// for contractions
